...
class Transformer_xomics(nn.Module):
    def __init__(self, config, xomic):
        ...
        self.encoder = Encoder(xomic.shape[2], config["num_head"], integrate_hidden, config["Transformer_drop"])
...
class Transformer_all(nn.Module):
    def __init__(self, config, all):
        ...
        self.encoder = Encoder(all.shape[2], config["num_head"], all_hidden, config["Transformer_drop"])
        self.encoders = nn.ModuleList([
            copy.deepcopy(self.encoder)
            for _ in range(config["Transformer_num_encoder"])])
        ...
        self.fc5 = nn.Linear(100, config["Transformer_output"])
...
class Transformer_Integrate(nn.Module):
    def __init__(self, config, Integrate):
        ...
        self.encoder = Encoder(Integrate.shape[2], config["num_head"], integrate_hidden, config["Transformer_drop"])
        self.encoders = nn.ModuleList([
            copy.deepcopy(self.encoder)
            for _ in range(config["Transformer_num_encoder"])])
        ...
        self.fc5 = nn.Linear(50, config["Transformer_output"])
...
class model_n_omics(nn.Module):
    def __init__(self, config, all_conc, omics_list, dict_omics:dict):
        super(model_n_omics, self).__init__()
        oms = list(dict_omics.keys())
        self.oms = oms
        self.net_all = Transformer_all(config, all_conc).to(device)
        self.omics = [Transformer_xomics(config, omics_list[oms.index(nkey)]).to(device) for nkey in oms]
    def forward(self, all_conc, omics_list):
        all_output, hidden_all = self.net_all(all_conc)
        outputs_and_hiddens = [self.omics[self.oms.index(xom)](omics_list[self.oms.index(xom)]) for xom in self.oms]
...
def test_n_omics(dataloader_te, model_te, config:dict, dict_omics:dict, regr_or_clas:int):
    y_hat_te_list = []
    ...
    for a, item_te in enumerate(dataloader_te):
        ...
        Model = model_n_omics(config, all_omics_te, to_device_omics, dict_omics).to(device)
...
def dataloader_and_model_pred(path_model:str, dict_omics:dict, regr_or_clas:int, batch_size:int=8):
    model = torch.load(path_model)
    omics_list = [intersect_omic(dict_omics[xom], path_model + "_" + xom + ".csv") for xom in list(dict_omics.keys())]

    dataset_te = dataset_n_omics(omics_list, "")
    dataloader_te = Dataloader.DataLoader(dataset_te, batch_size=batch_size, shuffle=False)

    y = test_n_omics(dataloader_te, model, config, dict_omics, regr_or_clas)
    return y


def dataloader_for_model_train(dict_omics:dict, path_labels:str, test_rate:float, regr_or_clas:int, batch_size:int):
    omics_and_labels = []
    omics = list(dict_omics.keys())
    for xi in omics:
        tmp_omic = pd.read_csv(dict_omics[xi], index_col=0)
        omics_and_labels.append(tmp_omic)
    ...
    omics_and_labels.append(labels)
    trte_omics_and_labels = train_test_split(*omics_and_labels, test_size=test_rate, shuffle=True, random_state=0)

    dataset_tr = dataset_n_omics(list(trte_omics_and_labels[:-2][::2]), trte_omics_and_labels[-2])
    dataloader_tr = Dataloader.DataLoader(dataset_tr, batch_size=batch_size, shuffle=True)

    dataset_te = dataset_n_omics(list(trte_omics_and_labels[:-2][1::2]), trte_omics_and_labels[-1])
    dataloader_te = Dataloader.DataLoader(dataset_te, batch_size=batch_size, shuffle=True)

    return dataloader_tr, dataloader_te


def train_val_n_omics_regr(dataloader_tr, dataloader_te, dict_omics:dict, path_model:str, epoch_max:int=540, patience:int=20, learning_rate:float=0.00001):
    ...
    for i in range(epoch_max):
        ...
        y_hat_tr_list = []
        total_tr_loss = 0
        Model = model_n_omics(config, all_omics_tr, to_device_omics, dict_omics).to(device)
        ...
        with torch.no_grad():
            ...
            r_val = computeCorrelation(y_hat_te_list, labels_te_list)
...
def train_val_n_omics_clas(dataloader_tr, dataloader_te, dict_omics:dict, path_model:str, epoch_max:int=540, patience:int=20, learning_rate:float=0.00001):
    config = read_config(path_model + ".cfg")
